...
def get_auth_token(role='user'):
    """Obtain JWT token for user or admin"""
    if role in _cached_tokens:
        return _cached_tokens[role]
    
    creds = {
        'user': ('demo_user', 'User@12345'),
        'admin': ('demo_admin', 'Admin@12345')
    }
    username, password = creds.get(role, creds['user'])
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as r:
            ...
            _cached_tokens[role] = payload.get('token')
            return _cached_tokens[role]
    except Exception as e:
        print(f"  [WARN] Failed to authenticate as {username}: {e}")
        return None
...
def log_check(category, item, passed, details=""):
    ...
    print(f"  {mark} {item.ljust(42)} : {status} {details}", flush=True)

def run_audit():
    print("=" * 70, flush=True)
    print("  EXECUTING COMPREHENSIVE 29-POINT VERIFICATION AUDIT", flush=True)
    print("=" * 70, flush=True)
    ...
    try:
        ...
        app_working = False
    log_check("BACKEND", "app.py created and working", app_file and app_working, f"(HTTP {status_code if status_code else 'ERR'})")

    # 1.2 All routes returning JSON
    routes_to_test = [
        ('/api/health', False),
        ('/', False),
        ('/api/analytics/overview', True),
        ('/api/analytics/comparison', True),
        ('/api/analytics/by-failure-type', True),
        ('/api/payments', True),
        ('/api/model/metrics', True)
    ]
    all_json = True
    for route, requires_auth in routes_to_test:
        try:
            headers = user_headers if requires_auth else {}
            req = urllib.request.Request(f"{BASE_BACKEND}{route}", headers=headers)
    ...
    # 1.6 Error handling implemented (validation rejection with 400 JSON)
    err_handled = False
    try:
    ...
    if sample_df is not None:
        ...
        cols_count = len(sample_df.columns)
    else:
        cols_count = 0
    log_check("ML SERVICE", "Generates realistic data", realistic_data, f"({cols_count} columns, realistic bounds)")
    ...
    try:
        ...
        prob_valid = False
    prob_str = f"{test_prob:.3f}" if isinstance(test_prob, float) else 'N/A'
    log_check("ML SERVICE", "predict_recovery_probability() returns 0-1", prob_valid, f"(Sample P = {prob_str})")
    ...
    action_str = analysis_res.get('recommended_action', 'N/A')
    id_str = analysis_res.get('transaction_id', 'N/A')
    log_check("INTEGRATION", "Gets response from backend", got_response, f"(ID: {id_str}, Action: {action_str})")

    # 4.4 Dashboard loads KPIs
    kpi_data = {}
    try:
        req = urllib.request.Request(f"{BASE_BACKEND}/api/analytics/overview", headers=user_headers)
        with urllib.request.urlopen(req, timeout=3) as r:
            kpi_data = json.loads(r.read().decode('utf-8'))
            kpi_loaded = ('total_analyzed' in kpi_data and 'recovery_rate' in kpi_data)
    except Exception:
        kpi_loaded = False
    log_check("INTEGRATION", "Dashboard loads KPIs", kpi_loaded, f"(Analyzed: {kpi_data.get('total_analyzed', 'N/A')}, Rate: {kpi_data.get('recovery_rate', 'N/A')}%)")

    # 4.5 Charts update with data
    comp_data = {}
    try:
        req = urllib.request.Request(f"{BASE_BACKEND}/api/analytics/comparison", headers=user_headers)
        with urllib.request.urlopen(req, timeout=3) as r:
            comp_data = json.loads(r.read().decode('utf-8'))
            charts_data = ('baseline' in comp_data and 'ai_model' in comp_data)
    except Exception:
        charts_data = False
    base_rate = comp_data.get('baseline', {}).get('recovery_rate', 'N/A')
    ai_rate = comp_data.get('ai_model', {}).get('recovery_rate', 'N/A')
    log_check("INTEGRATION", "Charts update with data", charts_data, f"(Baseline: {base_rate}%, AI: {ai_rate}%)")

    # 4.6 Transactions table populates
    txns = []
    try:
        req = urllib.request.Request(f"{BASE_BACKEND}/api/payments?limit=5", headers=user_headers)
        with urllib.request.urlopen(req, timeout=3) as r:
    ...
    metrics_data = {}
    try:
        req = urllib.request.Request(f"{BASE_BACKEND}/api/model/metrics", headers=user_headers)
        with urllib.request.urlopen(req, timeout=3) as r:
            metrics_data = json.loads(r.read().decode('utf-8'))
            metrics_ok = ('accuracy' in metrics_data and 'roc_auc' in metrics_data)
    except Exception:
        metrics_ok = False
    roc_str = metrics_data.get('roc_auc', 'N/A')
    acc_str = metrics_data.get('accuracy', 'N/A')
    log_check("INTEGRATION", "Model metrics display", metrics_ok, f"(ROC-AUC: {roc_str}, Accuracy: {acc_str})")
    ...
    # 5.1 Generate sample data works (Admin route)
    seed_res = {}
    try:
        ...
        seed_works = False
    log_check("TESTING", "Generate sample data works", seed_works, f"(Seeded {seed_res.get('count', 0)} transactions)")
    ...
    csv_text = ""
    try:
        req = urllib.request.Request(f"{BASE_BACKEND}/api/analytics/export", headers=user_headers)
        with urllib.request.urlopen(req, timeout=5) as r:
            csv_text = r.read().decode('utf-8')
            csv_works = (r.status == 200 and 'Transaction ID' in csv_text)
    except Exception:
        csv_works = False
    lines_count = len(csv_text.splitlines()) if csv_text else 0
    log_check("TESTING", "Export CSV works", csv_works, f"({lines_count} CSV lines downloaded)")

    # 5.4 Retrain model works (Admin route)
    retrain_res = {}
    try:
        headers = dict(admin_headers)
        headers['Content-Type'] = 'application/json'
        ...
        retrain_works = False
    retrain_auc = retrain_res.get('metrics', {}).get('roc_auc', 'N/A') if retrain_works else 'N/A'
    log_check("TESTING", "Retrain model works", retrain_works, f"(Retrained with ROC-AUC: {retrain_auc})")

    # 5.5 No console errors / clean encoding
    syntax_clean = True
